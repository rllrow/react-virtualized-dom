...
export function Tree() {
  ...
  const flatItems = useMemo(() => {
    const recurse = (items: TreeItem[], result: (Omit<TreeItem, 'children'> & { depth: number })[] = [], depth = 0) => {
      items.forEach((item) => {
  ...
  const renderItems = useCallback(
    ({ index, style }: ListChildComponentProps) => (
      <Row index={index} style={style} item={flatItems[index]} onExpand={onExpand} />
    ),
    [onExpand, flatItems],
  );
  ...
  return (
    <div className="tree">
      ...
      <AutoSizer>
        {({ width, height }) => (
          <FixedSizeList itemSize={ITEM_HEIGHT} itemCount={flatItems.length} width={width} height={height}>
            {renderItems}
          </FixedSizeList>
        )}
      </AutoSizer>
